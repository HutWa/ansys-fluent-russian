...
setlocal
set "QA_ROOT=%~dp0.."
set "QA_OUTPUT=%QA_ROOT%\build\visual-qa-cell-zone-conditions"
pushd "%QA_ROOT%"
if not exist "%QA_OUTPUT%" mkdir "%QA_OUTPUT%"
echo [%date% %time%] Opening only the Cell Zone Conditions task page through UIA. > "%QA_OUTPUT%\task-run.log"
"D:\Python312\python.exe" "%QA_ROOT%\scripts\navigate_fluent_uia.py" --target cell-zone-conditions --trace-file "%QA_OUTPUT%\navigation-trace.json" >> "%QA_OUTPUT%\task-run.log" 2>&1
