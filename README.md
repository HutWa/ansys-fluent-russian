...
**Переведено 27 104 из 28 786 строк Fluent — 94,2 % интерфейса Fluent Student 2026 R1.**
...
| Переведено | **27 104** |
| Требует контекста | 0 |
| Требует редакторской проверки | 0 |
| Не переводится | 1 682 |
| Ещё не добавлено в каталог проекта | 0 |
...
Условия выпуска и текущая готовность к первой бета-версии описаны в [плане beta-релиза](docs/BETA_RELEASE_V2026R1.md).
Известные ограничения и приоритеты повторной проверки собраны в [отдельном списке](docs/KNOWN_LIMITATIONS_V2026R1.md).

### Главное меню

**Лента Fluent (`Ribbon/*`): 1 150 из 1 150 строк переведены.** Это охватывает вкладки «Файл», «Домен», «Физика», «Пользовательские», «Решение», «Результаты», «Вид», «Параллельные вычисления», «Проектирование», «Параметрические» и «Обучение и поддержка», а также их команды.

Полный пакет `beta-1-27104-qa-fix-20260922` установлен в тестовую Fluent Student и проверен на копии реактора. Команды верхней ленты отображаются по-русски, но видимое дерево навигации всё ещё содержит смешанные русские и английские подписи. Первая бета-версия пока заблокирована девятью визуальными замечаниями; подробности — в [плане выпуска](docs/BETA_RELEASE_V2026R1.md).

## Автоматический контроль прогресса и визуальной проверки

Сценарии, приоритеты, условия безопасного запуска и команды повторной проверки описаны в [QA-сценариях](docs/QA_SCENARIOS_V2026R1.md). Они используют локальные снимки с SHA-256 и не требуют вручную пересылать скриншоты.
