# Каталоги переводов

Каждая версия Fluent хранится отдельно. `catalog.json` использует схему `schemas/translation-catalog.schema.json` и является исходным форматом проекта. `scripts/build_translation.py` преобразует его в Qt-каталоги `.qm` через `lrelease.exe` из локальной установки Fluent и записывает результат только во внешний staging-каталог. Исходные ресурсы Fluent в репозиторий не добавляются.

Пример записи первого прохода:

```json
{
  "id": "solver.example",
  "module": "SolverExample",
  "qt_context": "QObject",
  "source": "Example %1",
  "translation": "Пример %1",
  "status": "translated",
  "context": "Название панели или элемента UI",
  "comment": ""
}
```

Для `needs_review` укажите причину сомнения в `comment`, а возможные варианты — в `alternatives`. Для `needs_context` укажите, какого контекста интерфейса не хватает. Запись со статусом `reviewed` обязана содержать отдельный объект `review` с полями `reviewer` и `checked_at`.

`module` задаёт имя выходного `.qm`, `qt_context` — точный контекст Qt, а обычное поле `context` содержит понятное инженеру описание расположения и смысла строки.
